...
.DESCRIPTION
    This script performs a comprehensive health check of all domain controllers in a specified domain or across the entire Active Directory forest.

    It conducts a variety of diagnostics, including:
    - DNS resolution test (Resolve-DnsName)
    - ICMP ping reachability
    - Uptime and last reboot time
    - Time synchronization offset check (w32tm)
    - Disk space availability on OS drive (percentage and GB)
    - Service status verification (DNS, NTDS, NetLogon)
    - Full suite of DCDIAG tests (Connectivity, Replication, SysVol, FSMO roles, etc.)

    All results are compiled into a color-coded HTML report, making it easy to identify issues at a glance:
    - Green: Passed
    - Yellow: Warning
    - Red: Failed

    The script supports:
    - Saving the HTML report locally to a designated folder (default: C:\Reports)
    - Automatically sending the report via email using SMTP with the report attached and formatted as the HTML email body

    This tool is ideal for:
    - Scheduled health audits of Active Directory infrastructure
    - Daily/weekly monitoring of domain controllers
    - Troubleshooting domain controller replication and service issues

    Lightweight, modular, and easy to integrate with automation tools like Task Scheduler or Azure Automation.

.OUTPUTS
    A structured, color-coded HTML report saved to disk and/or emailed via SMTP.
...
$ReportFile = "$ReportPath\ADHealthReport_$reportFileName.html"
